...
export default function AdminSetupScreen({ onBack }) {
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/50 flex items-center justify-center p-6 text-slate-900 font-sans">
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white border border-slate-200 p-10 rounded-3xl shadow-2xl max-w-md w-full"
        >
          <div className="w-16 h-16 bg-indigo-50 text-indigo-650 border border-indigo-100 rounded-2xl flex items-center justify-center mb-6 mx-auto">
            <Lock size={32} />
          </div>
          <h1 className="text-2xl font-black text-slate-900 text-center mb-2">Accès Super Admin</h1>
          <p className="text-slate-500 text-center text-sm mb-8">Panneau de gestion BOUTIDIDACT.</p>
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            <input 
              ...
              className="w-full bg-slate-50 border border-slate-200 rounded-xl px-5 py-4 focus:ring-2 focus:ring-indigo-500/10 text-slate-900 placeholder:text-slate-400 outline-none transition"
              autoFocus
            />
            <button className="w-full bg-indigo-600 hover:bg-indigo-500 text-white py-4 rounded-xl font-bold transition shadow-lg shadow-indigo-600/20 active:scale-95">
              Déverrouiller
            </button>
            <button type="button" onClick={onBack} className="w-full text-slate-500 font-medium text-sm hover:text-slate-800 transition">
              Retour à l'accueil
            </button>
          </form>
        </motion.div>
  ...
  return (
    <div className="min-h-screen bg-slate-50/60 text-slate-900 font-sans">
      ...
      <div className="bg-white/80 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-gradient-to-tr from-indigo-600 to-fuchsia-600 text-white rounded-xl flex items-center justify-center">
              <Building2 size={20} />
            </div>
            <div>
              <h1 className="text-lg font-black text-slate-900 tracking-tight">BOUTIDIDACT</h1>
          ...
          <div className="flex items-center gap-2">
            <button 
              ...
              className="px-4 py-2 text-sm font-bold text-slate-500 hover:text-slate-900 transition flex items-center gap-2"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
        <div className="flex gap-1 bg-white p-1 rounded-xl border border-slate-250 w-fit shadow-sm">
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-12 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition font-medium"
          />
        </div>
        <button 
          onClick={fetchShops} 
          disabled={loading}
          className="px-5 py-3 bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 rounded-xl font-bold text-sm flex items-center gap-2 transition active:scale-95 disabled:opacity-50 shrink-0"
        >
...
function ShopCard({ shop, expanded, onToggle, password, onUpdate }) {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden transition-all hover:border-slate-350 shadow-sm mb-4">
      {/* Summary row */}
      <button 
        onClick={onToggle}
        className="w-full px-5 py-4 flex items-center gap-4 text-left transition-colors hover:bg-slate-50"
      >
        <div className="w-10 h-10 bg-indigo-50 border border-indigo-100 text-indigo-650 rounded-xl flex items-center justify-center shrink-0 font-black text-sm">
          {shop.name.charAt(0).toUpperCase()}
        </div>
        <div className="flex-1 min-w-0">
          <div className="font-bold text-slate-800 truncate">{shop.name}</div>
        ...
        <div className="hidden sm:flex items-center gap-3 shrink-0">
          {shop.paidAt && (
            <span className="px-2.5 py-1 bg-emerald-50 border border-emerald-250 text-emerald-700 rounded-lg text-xs font-bold">
              Actif
            </span>
          )}
          ...
          >
            <div className="px-5 pb-5 border-t border-slate-100 pt-4">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-xs font-black text-slate-500 uppercase tracking-widest">Fiche client</h4>
                <div className="flex gap-2">
                  {editing ? (
                    <>
                      <button onClick={() => setEditing(false)} className="px-3 py-1.5 text-xs font-bold text-slate-400 hover:text-slate-700 transition flex items-center gap-1">
                        <X size={14} /> Annuler
                      </button>
                      <button 
                        onClick={handleSave} 
                    ...
                    <button onClick={() => setEditing(true)} className="px-4 py-1.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-lg text-xs font-bold text-slate-700 flex items-center gap-1 transition">
              ...
              <div className="mt-4">
                ...
                {editing ? (
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-slate-55 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition font-medium text-sm resize-none"
                  />
                ) : (
                  <p className="text-sm text-slate-700 bg-slate-50 rounded-xl px-4 py-3 border border-slate-100 min-h-[60px]">
                    {form.notes || <span className="text-slate-500 italic">Aucune note</span>}
                  </p>
                )}
              </div>

              {/* Meta info */}
              <div className="mt-4 flex flex-wrap gap-2 text-[10px] font-bold text-slate-650">
                <span className="px-2 py-1 bg-slate-100 rounded">ID: {shop.id}</span>
                {shop.paidAt && (
                  <>
                    <span className="px-2 py-1 bg-emerald-50 text-emerald-700 border border-emerald-100 rounded">Début: {formatDate(shop.paidAt)}</span>
                    <span className="px-2 py-1 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded">Fin/Renouv.: {formatDate(new Date(shop.paidAt).getTime() + 30*24*60*60*1000)}</span>
                  </>
                )}
              </div>
            </div>
    ...
    <div className="space-y-1">
      ...
      {editing && onChange ? (
        <input
          ...
          className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-900 text-sm font-semibold outline-none focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 transition"
        />
      ) : (
        <p className="text-sm text-slate-800 font-semibold px-3 py-2">{value || <span className="text-slate-550">—</span>}</p>
      )}
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-xl">
        ...
        <div className="p-8">
          {success && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 p-4 bg-emerald-50 border border-emerald-200 rounded-xl flex items-center gap-3 text-emerald-700 text-sm font-bold">
              <CheckCircle size={20} /> E-mail envoyé avec succès !
            </motion.div>
          )}
          {error && (
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl flex items-center gap-3 text-red-700 text-sm font-bold">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="pt-4 border-t border-slate-100">
              <h3 className="text-xs font-black text-indigo-600 uppercase tracking-widest mb-4">Identifiants API</h3>
            ...
            <button 
              disabled={loading}
              className="w-full bg-indigo-600 hover:bg-indigo-750 disabled:bg-slate-100 disabled:text-slate-400 text-white py-4 rounded-xl font-black transition-all active:scale-95 shadow-lg shadow-indigo-600/15 mt-4 flex items-center justify-center gap-3"
            >
              {loading ? (
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        ...
        <input 
          ...
          className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder:text-slate-400 focus:ring-2 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition font-medium text-sm"
        />
      </div>
    </div>
  );
